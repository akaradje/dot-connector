 ...
 * Layer 7: The Scout             -> knowledge stops being one-way. Until now a dot could only be
 *                                   born from a human typing, a file dropped in inbox/, or the forge
 *                                   breaking a wall — even though the Evidence Agent searches the real
 *                                   web on every proof run and finds things that actually exist out
 *                                   there. Those finds were buried inside connections.json as footnotes
 *                                   to a verdict and never became connectable knowledge: the system
 *                                   searched the world and threw the result away, every single time.
 *                                   The Scout does two things. (1) It turns every similar_found into a
 *                                   real dot, tagged origin=scout:evidence with source evidence:<connId>,
 *                                   so nothing the system already paid to learn is discarded. (2) It reads
 *                                   its own domain census, names the domains it has no representative in,
 *                                   writes its own search queries, and goes out with WebSearch to fill
 *                                   those gaps by itself. Every dot now carries an origin, and anything
 *                                   the system fetched on its own can be vetoed by the owner — a veto the
 *                                   Scout remembers, so the repository keeps growing while it stays yours.
 *
...
/* ================= Layer 7: THE SCOUT =================
 * The oldest assumption in this codebase was that knowledge is something the owner
 * carries in. harvestText() was reachable from exactly two places — processInbox() and
 * POST /api/capture — both of which begin with a human act. Meanwhile the Evidence Agent
 * ran a real web search on every proof round and came back with `similar_found`: things
 * that genuinely exist in the world, named and described. That list was written into
 * connections.json as a footnote to a verdict, rendered once in renderEvidence(), and
 * read by nothing else, ever. The system paid to look at the world and then binned what
 * it saw — every single time.
 *
 * The Scout closes both halves of that:
 *   · harvestEvidenceDots() promotes every similar_found into a first-class dot, tagged
 *     origin=scout:evidence and source=evidence:<connId> so it never masquerades as the
 *     owner's own knowledge. Pure bookkeeping — no AI, no network, idempotent.
 *   · scoutGaps() reads the repository's own domain census against a reference set of
 *     broad human domains and names the ones with no representative at all, writing its
 *     own search query for each. scoutWeb() then takes the top gap and goes out with
 *     WebSearch to bring dots back from a domain the owner never entered.
 *
 ...
 * never re-add what the owner threw out. The repository stays theirs — it just stops
 * standing still while they are busy.
 */
 ...
 * The wall this layer breaks is the one that produced all the others: the fitness function
 * above only knows how to say yes to growth. `out.differential.ok = !oldProof.ok` demands
 * that the proof FAIL on yesterday's code, so any change whose whole point is that
 * behaviour stays identical — merging duplicate code, deleting what nothing calls,
 * simplifying a path, dropping a dead endpoint — is unprovable by construction: its test
 * passes on both trees, which the differential gate reads as "you did nothing". Meanwhile
 * REGRESSION_ENDPOINTS could only ever be appended to, so every round added mass to the
 * sweep, to the source, and to the self-forge prompt, and the engine's own context ceiling
 * came closer with every success. One direction, no brakes.
 *
 * So there are now two round types, with two fitness functions that are exact inversions
 * of each other:
 *
 ...
 *
 * A consolidation round is not the soft option. It re-runs the *whole accumulated suite*
 * (every selftest/*.js an earlier round ever shipped) against both trees and demands an
 * identical pass/fail signature file by file: one flipped result and the round is rolled
 * back. On top of that it must move a number — fewer code lines, fewer code files, a
 * smaller forge prompt, or fewer live endpoints — with no growth in any size metric.
 * The full regression sweep still runs. Nothing here is a weakened gate; it is a different
 * gate, pointed the other way.
 *
 * Retiring an endpoint is the one destructive power in the system, so it is the most
 * heavily fenced: every /api/* request increments a live usage ledger, and a path can be
 * retired only while its hit count is exactly zero. Retirement is recorded with who did it
 * and when, is undone with one call, and never deletes the historical declaration.
 */
    ...
    if (!changes.length) {
      ...
      if (!syn.ok) {
        ...
        if (!smoke.ok) {
          ...
          // The inverted fitness function. Same three questions as an expansion round,
          // two of them asked the other way round: did *everything* keep working, and
          // did the engine actually get smaller?
          retirePlan = planRetirements(report.retire_endpoints);
          const proof = await proveConsolidation(evoId, before, after, guarded, { retiring: retirePlan.approved });
          entry.checks.preservation = proof.preservation.ok;
          entry.checks.preservation_detail = proof.preservation.detail;
          entry.checks.shrink = proof.shrink.ok;
          entry.checks.shrink_detail = proof.shrink.detail;
          ...
          const failed = [
            !proof.preservation.ok && "พฤติกรรมไม่เหมือนเดิม (ชุดทดสอบต้องผ่านทั้งก่อนและหลัง)",
            !proof.shrink.ok && "ไม่ได้เล็กลงจริงตามตัวเลข",
