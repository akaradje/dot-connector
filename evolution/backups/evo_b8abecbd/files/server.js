...
function buildForgePrompt(limit, body, evoId) {
...
async function attemptEvolution({ limitId = null, auto = false } = {}) {
  ...
  try {
    ...
    // 2. Let it rewrite itself — and make it write the proof of its own new capability.
    fs.mkdirSync(PROOF_DIR, { recursive: true });
    const raw = await runClaude(buildForgePrompt(target, before, evoId), {
      model: FORGE_MODEL,
